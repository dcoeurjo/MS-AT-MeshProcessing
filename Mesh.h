...
namespace DDG
{
    class Mesh
    {
        public:
            ...
            int writeMesh( const std::string& filename ) const;
            // writes a mesh to a Wavefront OBJ file; return value is nonzero
            // only if there was an error

            int writeEdgeTube( const std::string& filename, const double radius, const int faces ) const;
            int writeEdgeStrips( const std::string& filename) const;
            int writeEdgeCSV( const std::string& filename ) const;
